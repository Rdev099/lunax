  ...
  const money = (usd) => (window.LUNAX && typeof LUNAX.formatMoney === 'function')
    ...
    let flashEnd = Date.now() + 6 * 3600 * 1000;   // sale lasts 6 hours, then restarts
let flashTimerId;

const colsNow = () =>
  window.innerWidth <= 640 ? 1 : window.innerWidth <= 800 ? 2 : window.innerWidth <= 1180 ? 3 : 5;
...
<div class="flash-panel">
  <div class="flash-left">
    ...
    <h3>Up to 50% off</h3>
    <p>Limited-time deals on top electronics.</p>
  </div>
  <div class="flash-timer" id="flashTimer">
    <div><b data-u="h">00</b><span>Hours</span></div>
    <div><b data-u="m">00</b><span>Mins</span></div>
    <div><b data-u="s">00</b><span>Secs</span></div>
  </div>
  <a href="#" class="flash-btn">Shop deals</a>
</div>`;
}
...
function startFlashTimer(){
  ...
  const tick = () => {
    ...
    if (left === 0) flashEnd = Date.now() + 6 * 3600 * 1000;
    const set = (u, v) => {
      const el = document.querySelector(`#flashTimer [data-u="${u}"]`);
      if (el) el.textContent = String(v).padStart(2, '0');
    };
    set('h', Math.floor(left / 3600000));
    set('m', Math.floor((left % 3600000) / 60000));
    set('s', Math.floor((left % 60000) / 1000));
  };
  tick();
...
<div class="product-card">
  ...
  ${p.image
    ? `<img src="${esc(p.image)}" alt="${esc(title.trim())}" loading="lazy">`
  ...
  <div class="product-info">
    ...
    <div class="card-actions">
      <button class="add-btn" data-id="${esc(p.id)}" data-index="${p.origIndex}" data-name="${esc(p.name)}" data-price="${p.price}" data-image="${esc(p.image || '')}" data-icon="${esc(p.icon || '')}">Add to Cart</button>
    ...
    const cols = colsNow();
    if (cards.length > cols) cards.splice(cols, 0, flashPanelHTML());  // panel after the first row
